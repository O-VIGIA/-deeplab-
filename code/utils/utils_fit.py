...
from HWCV_Final.nets.deeplabv3_training import CE_Loss, Dice_loss, Focal_Loss
from HWCV_Final.utils.utils import get_lr
from HWCV_Final.utils.utils_metrics import f_score
...
def fit_one_epoch(model_train, model, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch,
    ...
    # 可视化
    mean_train_loss = total_loss / (epoch_step + 1)
    mean_val_loss = val_loss / (epoch_step_val + 1)
    mean_train_f_score = total_f_score / (epoch_step + 1)
    mean_val_f_score = val_f_score / (epoch_step_val + 1)
    return mean_train_loss, mean_train_f_score, mean_val_loss, mean_val_f_score